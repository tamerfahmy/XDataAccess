...
namespace XDataAccess.QueryBuilder.Tests.MySql
{
    public class DeleteTests
    {
        ...
        public void Delete_ContainsCondition_Test()
        {
            ...
            Assert.Equal("DELETE FROM Employee WHERE (`Name` LIKE CONCAT('%' , @P0 , '%'))", result.SqlQuery);
        }
        ...
        public void Delete_ContainsWithAndCondition_Test()
        {
            ...
            Assert.Equal(1, result.QueryParameters["@P0"]);
            Assert.Equal("test", result.QueryParameters["@P1"]);
            Assert.Equal("DELETE FROM Employee WHERE ((`Id` = @P0) AND (`Name` LIKE CONCAT('%' , @P1 , '%')))", result.SqlQuery);
        }
